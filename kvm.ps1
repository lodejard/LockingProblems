...
function Kvm-Global-Setup {
  ...
  If (Needs-Elevation)
  {
    ...
    WriteLine "Adding $kvmBinPath to process PATH"
    Set-Path (Change-Path $env:Path $kvmBinPath ($kvmBinPath))
    WriteLine "Adding $globalKrePath;%USERPROFILE%\.kre to process KRE_HOME"
  ...
  WriteLine "Copying file $kvmBinPath\kvm.ps1"
  md $kvmBinPath -Force | Out-Null
  copy "$scriptFolder\kvm.ps1" "$kvmBinPath\kvm.ps1"

  WriteLine "Copying file $kvmBinPath\kvm.cmd"
  copy "$scriptFolder\kvm.cmd" "$kvmBinPath\kvm.cmd"

  WriteLine "Adding $kvmBinPath to process PATH"
  Set-Path (Change-Path $env:Path $kvmBinPath ($kvmBinPath))

  WriteLine "Adding $kvmBinPath to user PATH"
  ...
  WriteLine "Adding $globalKrePath;%USERPROFILE%\.kre to machine KRE_HOME"
...
param(
  ...
  WriteLine "Determining latest version"
...
param(
  ...
  If (Test-Path $kreFolder) {
    if($force)
    {
      ...
      WriteLine "$kreFullName already installed."
      return;
    }
  }

  WriteLine "Downloading $kreFullName from $feed"
  ...
  WriteLine "Installing to $kreFolder"
...
param(
  ...
  WriteLine "Unpacking to $kreFolder"
...
param(
  ...
  if ($versionOrAlias.EndsWith(".nupkg")) {
    ...
    if ($folderExists) {
      WriteLine "Target folder '$kreFolder' already exists"
    } else {
...
param(
  ...
  if ($versionOrAlias -eq "none") {
    if ($persistent) {
      WriteLine "Removing KRE from machine PATH"
  ...
  if ($kreBin -eq $null) {
    WriteLine "Cannot find $kreFullName, do you need to run 'kvm install $versionOrAlias'?"
    return
  }

  if ($persistent) {
    WriteLine "Adding $kreBin to machine PATH"
    $machinePath = [Environment]::GetEnvironmentVariable("Path", [System.EnvironmentVariableTarget]::Machine)
...
param(
  ...
  if ($versionOrAlias -eq "none") {
    WriteLine "Removing KRE from process PATH"
  ...
  WriteLine "Adding" $kreBin "to process PATH"
...
param(
  ...
  if ($versionOrAlias -eq "none") {
    ...
    if ($persistent) {
      WriteLine "Removing KRE from user PATH"
      $userPath = [Environment]::GetEnvironmentVariable("Path", [System.EnvironmentVariableTarget]::User)
  ...
  if ($persistent) {
    WriteLine "Adding $kreBin to user PATH"
    $userPath = [Environment]::GetEnvironmentVariable("Path", [System.EnvironmentVariableTarget]::User)
...
param(
  ...
  if (!(Test-Path $aliasFilePath)) {
    WriteLine "Alias '$name' does not exist"
  } else {
    WriteLine "Alias '$name' is set to" (Get-Content ($userKrePath + "\alias\" + $name + ".txt"))
  }
}
...
param(
  ...
  WriteLine "$action alias '$name' to '$kreFullName'"
  md ($userKrePath + "\alias\") -Force | Out-Null
...
param(
  ...
  if (Test-Path -literalPath "$aliasPath") {
      WriteLine "Removing alias $name"
      Remove-Item -literalPath $aliasPath
  } else {
      WriteLine "Cannot remove alias, '$name' is not a valid alias name"
  }
}
...
function Validate-And-Santitise-Switches()
{
  ...
  if ($runtime) {
    ...
    WriteLine "Warning: -svr50 is deprecated, use -runtime CLR for new packages."
    Set-Variable -Name "selectedRuntime" -Value "svr50" -Scope Script
  } elseif ($svrc50) {
    WriteLine "Warning: -svrc50 is deprecated, use -runtime CoreCLR for new packages."
    Set-Variable -Name "selectedRuntime" -Value "svrc50" -Scope Script
  }

  if ($x64) {
    WriteLine "Warning: -x64 is deprecated, use -amd64 for new packages."
...
try {
  ...
  $_
#  WriteLine $_ -ForegroundColor Red ; 
  "Type 'kvm help' for help on how to use kvm."
}
if ($wait) {
